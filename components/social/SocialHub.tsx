...
type HubView = 'feed' | 'vlog' | 'routes' | 'events' | 'profile';
...
export const SocialHub: React.FC<SocialHubProps> = ({ user: propUser, onNavigate, onLogout, onUpdateUser, initialData }) => {
    ...
    return (
        <div className="bg-[#050505] min-h-screen text-white pt-20">
            <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-8 h-[calc(100vh-80px)]">

                {/* LEFT SIDEBAR (Navigation) */}
                <div className="hidden lg:col-span-3 lg:flex flex-col gap-2 py-6 sticky top-20 h-full z-20">
                    ...
                    <div className="mt-auto">
                        {/* Premium Ad / Event Widget (Moved to bottom of nav for better space usage in map views) */}
                        <div className="relative overflow-hidden rounded-3xl aspect-[2/1] group cursor-pointer border border-white/10 hidden xl:block">
                            <img src="https://images.unsplash.com/photo-1558981806-ec527fa84d3d?q=80&w=800" alt="Event" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-60" />
                            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent flex flex-col justify-end p-4">
                                <span className="text-moto-accent text-[10px] font-black uppercase tracking-widest mb-1">GELECEK ETKİNLİK</span>
                                <h3 className="text-lg font-display font-black text-white leading-none">GECE SÜRÜŞÜ</h3>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {view === 'feed' ? (
                    ...
                    /* WIDE VIEW AREA for Maps/Routes/Events */
                    <div className="lg:col-span-9 h-full relative overflow-hidden rounded-3xl border border-white/10 bg-[#0f0f0f]">
                        {view === 'vlog' && (
                            <MotoVlogMap
                                user={currentUser}
                        ...
                        {view === 'routes' && (
                            <RouteExplorer
                                user={currentUser}
                                onOpenAuth={() => onNavigate && onNavigate('auth')}
                                // @ts-ignore
                                isEmbedded={true}
                            />
                        )}
