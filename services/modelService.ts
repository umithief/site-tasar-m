...
export const modelService = {
    ...
    async addModel(model: Omit<Model3DItem, '_id'>): Promise<Model3DItem> {
        if (CONFIG.USE_MOCK_API) {
            await delay(500);
    ...
    async updateModel(model: Model3DItem): Promise<void> {
        if (CONFIG.USE_MOCK_API) {
            await delay(300);
            const models = getStorage<Model3DItem[]>(DB.MODELS, []);
            const index = models.findIndex(m => m._id === model._id);
            if (index !== -1) {
                models[index] = model;
                setStorage(DB.MODELS, models);
            }
        } else {
            await fetch(`${CONFIG.API_URL}/models/${model._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(model)
            });
    ...
    async deleteModel(id: string): Promise<void> {
        if (CONFIG.USE_MOCK_API) {
            await delay(300);
            const models = getStorage<Model3DItem[]>(DB.MODELS, []);
            const filtered = models.filter(m => m._id !== id);
