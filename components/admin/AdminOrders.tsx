    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-right duration-300">
            <div className="space-y-4">
                {orders.map(order => (
                    <div key={order._id} className="bg-[#1A1A17] border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-all">
                        <div className="flex justify-between items-center mb-4">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center text-[#F2A619] font-bold text-xs border border-white/5">#{order._id.slice(-4)}</div>
                                <div>
                        ...
                        <div className="mt-4 flex gap-2 justify-end">
                            {['Hazırlanıyor', 'Kargoda', 'Teslim Edildi'].map(s => (
                                <button key={s} onClick={() => handleOrderStatusChange(order._id, s)} className={`px-3 py-1 rounded-lg text-xs font-bold border transition-all ${order.status === s ? 'bg-white text-black border-white' : 'border-white/10 text-gray-500 hover:text-white'}`}>{s}</button>
                            ))}
                        </div>
                    </div>
                ))}
