        ...
        <div className="space-y-6 animate-in slide-in-from-right duration-300">
            <div className="space-y-4">
                {negotiations.map(offer => (
                    <div key={offer._id} className="bg-[#1A1A17] border border-white/5 rounded-2xl p-5 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            {offer.status === 'pending' ? (
                                <div className="flex gap-2">
                                    <button onClick={() => handleNegotiationAction(offer._id, 'accepted')} className="bg-green-600 hover:bg-green-500 text-white p-2 rounded-lg transition-colors"><Check className="w-5 h-5" /></button>
                                    <button onClick={() => handleNegotiationAction(offer._id, 'rejected')} className="bg-red-600 hover:bg-red-500 text-white p-2 rounded-lg transition-colors"><X className="w-5 h-5" /></button>
                                </div>
                            ) : (
