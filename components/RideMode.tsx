...
const handleGpsError = (err: GeolocationPositionError) => {
    ...
    if (err.code === 1) msg = "Konum izni reddedildi. Lütfen tarayıcı ayarlarından izin verin. (Telefonda HTTPS gereklidir)";
    else if (err.code === 2) msg = "Konum bulunamadı. Lütfen açık alanda deneyin.";
    else if (err.code === 3) msg = "Konum alma zaman aşımına uğradı.";

    // Non-intrusive alert for mobile debugging
...
export const RideMode: React.FC<RideModeProps> = ({ route, onNavigate }) => {
    ...
    useEffect(() => {
        if (mapContainerRef.current && !mapRef.current && typeof L !== 'undefined') {
            ...
            // CARTO VOYAGER TILES (Clean, Pastel, Modern)
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                (err) => { handleGpsError(err); setNavMessage("GPS Hatası: " + err.message); },
        ...
        import('socket.io-client').then(({ io }) => {
            ...
            socket.on('rider_left', (data: { userId: string }) => {
                ...
                if (otherRidersRef.current[targetId]) {
                    otherRidersRef.current[targetId].remove();
                    delete otherRidersRef.current[targetId];
                }
            });
        });
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-gray-50/10 text-slate-800 flex flex-col font-sans select-none overflow-hidden h-[100dvh]">
            ...
            <div className="absolute inset-0 z-0 overflow-hidden">
                ...
                {/* Optional: Subtle vignette if needed, but keeping it clean for now */}
            </div>
            ...
            <div className="absolute top-0 left-0 right-0 h-16 pt-safe-top z-50 flex justify-between items-center px-4 pointer-events-none bg-gradient-to-b from-black/90 to-transparent">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 top-4 flex flex-col items-center">
                    <span className="text-xl font-display font-black text-slate-800 tracking-wider drop-shadow-sm glass-panel px-4 py-1">
