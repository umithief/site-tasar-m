...
export const PostCard: React.FC<PostCardProps> = ({ post, currentUserId, onNavigate, onCommentClick }) => {
    ...
    return (
        <motion.div
            ...
            className="w-full max-w-2xl mx-auto bg-[#0D0D0D] dark:bg-[#0D0D0D] bg-white rounded-[24px] border border-white/5 dark:border-white/5 border-gray-100 shadow-2xl dark:shadow-2xl shadow-gray-200/50 overflow-hidden mb-8 group"
        >
            {/* Header */}
            <div className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3 cursor-pointer" onClick={() => onNavigate && onNavigate('public-profile', { _id: post.userId })}>
                    ...
                    <div>
                        <h3 className="text-white dark:text-white text-gray-800 font-bold text-sm leading-none">{post.userName}</h3>
            ...
            <div className="p-5 pt-2">
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <div className={`p-2 rounded-full transition-colors ${isLiked ? 'bg-[#FF3E3E]/10' : 'bg-white/5 group-hover/like:bg-white/10'}`}>
                                <Heart
                                    className={`w-5 h-5 transition-all ${isLiked ? 'fill-[#FF3E3E] text-[#FF3E3E]' : 'text-white dark:text-white text-gray-500'}`}
                                />
                            </div>
                        ...
                        <button
                            ...
                            className="group/comment flex items-center gap-1.5 text-zinc-500 hover:text-white dark:hover:text-white hover:text-gray-900 transition-colors"
                        >
                            <div className="p-2 rounded-full bg-white/5 dark:bg-white/5 bg-gray-100 group-hover/comment:bg-white/10 dark:group-hover/comment:bg-white/10 group-hover/comment:bg-gray-200 transition-colors">
                    ...
                    <button className="text-zinc-500 hover:text-white dark:hover:text-white hover:text-gray-900 transition-colors">
                ...
                <div className="space-y-2">
                    <p className="text-zinc-300 dark:text-zinc-300 text-gray-600 text-sm leading-relaxed">
                        {post.content}
                    </p>
