...
export const RideMode: React.FC<RideModeProps> = ({ route, onNavigate }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black text-white flex flex-col font-sans select-none overflow-hidden h-[100dvh]">
            ...
            {/* --- CENTER HUD INFO (Turn-by-Turn & G-Force) --- */}
            <div className="absolute top-28 left-0 right-0 z-40 pointer-events-none px-6">
                <div className="flex justify-between items-start">
                    {/* Left: Turn Instructions */}
                    <div className="space-y-4 max-w-[50%]">
                        {/* Dynamic Active Target */}
                        {activeTarget && (
                            <div className="bg-black/60 backdrop-blur-md border border-cyan-500/30 p-4 rounded-r-2xl border-l-4 border-l-cyan-400 shadow-[0_0_30px_rgba(34,211,238,0.1)] animate-in fade-in slide-in-from-left-4">
                                <div className="text-cyan-400 text-xs font-bold uppercase tracking-wider mb-1 flex items-center gap-2">
                                    <div className="w-2 h-2 bg-cyan-400 rounded-full animate-pulse"></div>
                                    Aktif Hedef
                                </div>
                                <div className="text-2xl font-black text-white leading-none mb-1">{activeTarget.name}</div>
                                <div className="text-lg text-cyan-200 font-mono">
                                    {(activeTarget.distance).toFixed(1)} <span className="text-sm">km</span>
                                </div>
                            </div>
                        )}

                        {nextTurn && (
                            <div className="bg-black/80 backdrop-blur-xl border border-white/10 p-5 rounded-2xl shadow-2xl animate-in fade-in slide-in-from-left-4 max-w-sm">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-zinc-400 font-medium leading-tight">{nextTurn.text}</div>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    {/* Right: Telemetry (Visibility Toggle) */}
                    {showTelemetry && !isLowPowerMode && (
                        <div className="flex flex-col gap-2 items-end animate-in fade-in slide-in-from-right-4">
                            <div className="relative w-32 h-32">
                                {/* G-Force Circle */}
                                <svg className="w-full h-full transform -rotate-90">
                                    <circle cx="64" cy="64" r="60" stroke="rgba(255,255,255,0.1)" strokeWidth="2" fill="none" />
                                    <circle cx="64" cy="64" r="30" stroke="rgba(255,255,255,0.1)" strokeWidth="1" fill="none" />
                                    <line x1="64" y1="4" x2="64" y2="124" stroke="rgba(255,255,255,0.1)" strokeWidth="1" />
                                    <line x1="4" y1="64" x2="124" y2="64" stroke="rgba(255,255,255,0.1)" strokeWidth="1" />
                                </svg>
                                {/* Active G-Dot */}
                                <div
                                    ...
                                    style={{
                                        left: '50%',
                                        top: '50%',
                                        transform: `translate(calc(-50% + ${Math.min(Math.max(telemetry.gForce * 30, -50), 50)}px), -50%)`
                                    }}
                                ></div>
                                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 text-[10px] text-zinc-500 font-bold tracking-widest">G-FORCE</div>
                            </div>
                            <div className="bg-black/40 backdrop-blur border border-white/5 p-3 rounded-xl min-w-[120px]">
                ...
                <div className="relative flex items-end justify-center pb-8 px-8 gap-12 bg-gradient-to-t from-black via-black/90 to-transparent pt-32">
                    ...
                    <div className="relative group cursor-pointer" onClick={() => setShowTelemetry(!showTelemetry)}>
                        ...
                        <div className="relative">
                            ...
                            <span className="absolute -right-4 top-4 text-xl font-bold text-zinc-500 rotate-12">KMH</span>
                        </div>
                    ...
                    <div className="hidden md:flex flex-col items-start gap-4 w-32">
                        ...
                        <button
                            ...
                            className="pointer-events-auto group flex items-center gap-3 pl-1 pr-4 py-1.5 bg-black/40 hover:bg-white/10 border border-white/10 rounded-full transition-all backdrop-blur-md"
                        >
                            <div className="w-8 h-8 rounded-full bg-cyan-950 text-cyan-400 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                                <Search size={14} strokeWidth={3} />
                            </div>
                            <span className="text-xs font-bold text-zinc-300">Hızlı Git</span>
                        </button>
