    ...
    >
      <div className="flex items-center justify-between p-3 bg-white dark:bg-[#151515] border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm relative z-10">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 flex items-center justify-center rounded-xl border border-gray-100 dark:border-white/5 ${weather.isDay ? 'bg-[#E2FF3B]/10 dark:bg-[#E2FF3B]/10' : 'bg-blue-50 dark:bg-blue-500/10'}`}>
            <Icon className={`w-6 h-6 ${weather.isDay ? 'text-[#E2FF3B]' : 'text-blue-400'}`} />
          </div>
          <div>
      ...
      {/* Ambient Glow */}
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full opacity-20 blur-xl rounded-full pointer-events-none transition-colors ${weather.isDay ? 'bg-[#E2FF3B]/20' : 'bg-blue-500/20'}`}></div>
    </motion.div>
  );
};
