...
export const MobilePostCard: React.FC<MobilePostCardProps> = ({ post, currentUserId, onLike, onComment }) => {
    ...
    return (
        <motion.div
            ...
            className="w-full bg-white mb-4 relative border-b border-gray-100 pb-6 shadow-sm"
        >
            {/* Cinematic Header (Floating over image) */}
            <div className="absolute top-0 left-0 right-0 z-20 p-4 bg-gradient-to-b from-white/90 via-white/40 to-transparent pointer-events-none">
                <div className="flex items-center justify-between pointer-events-auto">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <div className="w-10 h-10 rounded-full p-[1.5px] bg-gradient-to-tr from-moto-accent to-gray-200">
                                <img
                        ...
                        <div>
                            <h3 className="text-gray-900 text-sm font-bold font-display drop-shadow-sm">{post.userName}</h3>
                    ...
                    <button className="w-8 h-8 rounded-full bg-white/50 backdrop-blur-md border border-gray-100 flex items-center justify-center text-gray-900 active:bg-gray-100">
                ...
                {post.rideStats && (
                    <div className="absolute bottom-4 left-4 right-4 z-20">
                        <div className="grid grid-cols-3 gap-2">
                            <div className="bg-white/80 backdrop-blur-md rounded-xl p-2 flex flex-col items-center justify-center border border-gray-100 shadow-sm">
                                <Gauge className="w-3 h-3 text-moto-accent mb-1" />
                                <span className="text-lg font-mono font-bold text-gray-900 leading-none">{post.rideStats.maxSpeed}</span>
                                <span className="text-[8px] text-gray-500 uppercase font-bold tracking-wider mt-0.5">KM/H</span>
                            </div>
                            <div className="bg-white/80 backdrop-blur-md rounded-xl p-2 flex flex-col items-center justify-center border border-gray-100 shadow-sm">
                                <Navigation className="w-3 h-3 text-moto-accent mb-1" />
                                <span className="text-lg font-mono font-bold text-gray-900 leading-none">{post.rideStats.distance}</span>
                                <span className="text-[8px] text-gray-500 uppercase font-bold tracking-wider mt-0.5">KM</span>
                            </div>
                            <div className="bg-white/80 backdrop-blur-md rounded-xl p-2 flex flex-col items-center justify-center border border-gray-100 relative overflow-hidden shadow-sm">
                                <span className="text-[8px] text-gray-500 uppercase font-bold tracking-wider relative z-10">ROTA</span>
            ...
            <div className="px-5 relative z-20">
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={handleLike} className="group outline-none">
                            <div className={`p-2.5 rounded-full transition-all duration-300 border ${isLiked ? 'bg-moto-accent/10 border-moto-accent/50' : 'bg-transparent border-transparent'}`}>
                                <Heart className={`w-6 h-6 transition-all ${isLiked ? 'text-moto-accent fill-moto-accent scale-110 drop-shadow-[0_0_10px_rgba(226,255,59,0.4)]' : 'text-gray-900'}`} strokeWidth={isLiked ? 0 : 2} />
                            </div>
                        </button>
                        ...
                        <button className="p-2.5 rounded-full outline-none active:scale-90 transition-transform">
                            <Share2 className="w-6 h-6 text-gray-900" strokeWidth={2} />
                        </button>
                    </div>
                    <button className="p-2.5 outline-none active:scale-90 transition-transform">
                        <Bookmark className="w-6 h-6 text-gray-900" strokeWidth={2} />
                    </button>
                </div>
                ...
                <div className="space-y-1.5 px-1">
                    ...
                    <div className="text-sm text-gray-600 leading-relaxed">
                        <span className="font-bold text-gray-900 mr-2 font-display">{post.userName}</span>
                        {post.content}
                    </div>
