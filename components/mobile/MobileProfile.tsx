    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-24 transition-colors duration-300">
                        ...
                        <div className={`w-full h-full flex items-center justify-center bg-gray-200`}>
                            ...
                            <div className="flex flex-col items-center opacity-30">
                                <div className="w-16 h-16 text-gray-400 mb-2">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full text-current fill-current"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>
                                </div>
                            </div>
                        </div>
                    )}
            ...
            <div className="px-4 -mt-16 relative z-10">
                <div className="bg-white rounded-[2rem] p-6 shadow-xl shadow-gray-200/50 border border-gray-100 text-center transition-colors">
                    <div className="relative inline-block mb-3">
                        <UserAvatar name={user.name} src={user.avatar || user.profileImage} size={100} className="border-4 border-white shadow-lg" />
                        <div className="absolute bottom-0 right-0 bg-moto-accent rounded-full p-1.5 border-4 border-white">
                            <Shield className="w-3 h-3 text-black fill-current" />
                        </div>
                    </div>

                    <h1 className="text-2xl font-black text-gray-900 mb-1 tracking-tight">{user.name}</h1>
                    <p className="text-xs font-bold text-gray-500 mb-4 bg-gray-50 inline-block px-3 py-1 rounded-full border border-gray-100">
                        @{user.username || 'rider'}
                    </p>
            ...
            <div className="sticky top-0 z-30 bg-gray-50 pt-4 pb-2 mt-2 px-4 overflow-x-auto no-scrollbar backdrop-blur-xl bg-opacity-90">
                <div className="flex gap-2 justify-center bg-white p-1.5 rounded-2xl border border-gray-200 shadow-sm">
                    {[
                    ...
                    >
                        ...
                        {activeTab === 'reels' && (
                            <div className="grid grid-cols-3 gap-1">
                                {MOCK_REELS.map(reel => (
                                    <div key={reel.id} className="relative aspect-[9/16] bg-gray-200 overflow-hidden">
                            ...
                            <div className="space-y-4 px-2">
                                {user.garage?.map(bike => (
                                    <div key={bike._id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex gap-4">
                                        ...
                                        <div>
                                            <h3 className="font-black text-gray-900 text-lg leading-tight">{bike.brand}</h3>
                                            <p className="text-sm text-gray-500 mb-2">{bike.model}</p>
                                            <div className="flex gap-2">
                                                <span className="px-2 py-1 bg-gray-50 rounded-md text-[10px] font-bold text-gray-600 border border-gray-100">{bike.km} KM</span>
                                                <span className="px-2 py-1 bg-gray-50 rounded-md text-[10px] font-bold text-gray-600 border border-gray-100">{bike.year}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
