...
export const CategoryGrid: React.FC<CategoryGridProps> = ({ onCategorySelect }) => {
    ...
    return (
        <section className="py-8 md:py-24 bg-gray-50 dark:bg-black relative overflow-hidden transition-colors duration-500">
                ...
                <div className="px-4 md:px-8 flex flex-col md:flex-row md:items-end justify-between mb-6 md:mb-12 gap-2 md:gap-6">
                    <div className="flex items-center justify-between w-full">
                        <div>
                            ...
                            <h2 className="text-2xl md:text-6xl font-display font-black text-gray-900 dark:text-white tracking-tight leading-none">
                                EKİPMANINI <span className="text-transparent bg-clip-text bg-gradient-to-r from-moto-accent to-orange-500">SEÇ</span>
                            </h2>
                        </div>

                        {/* Mobile "See All" Link */}
                        <button className="md:hidden text-xs font-bold text-gray-500 dark:text-gray-400 flex items-center gap-1">
                ...
                <div className="md:hidden flex overflow-x-auto gap-4 px-4 pb-4 snap-x snap-mandatory no-scrollbar scroll-pl-4 pr-4">
                    {categories.map((cat, idx) => {
                        ...
                        return (
                            <motion.div
                                key={cat._id}
                                className="snap-center min-w-[150px] h-[220px] relative rounded-2xl overflow-hidden bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/10 group shadow-sm"
                            ...
                            >
                                ...
                                <div className="absolute top-3 right-3 w-8 h-8 bg-white/20 dark:bg-black/30 backdrop-blur rounded-full flex items-center justify-center border border-white/20">
                    ...
                    {categories.map((cat, idx) => {
                        ...
                        return (
                            <motion.div
                                ...
                                onClick={() => onCategorySelect(cat.type)}
                                className={`group relative overflow-hidden rounded-3xl cursor-pointer bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/10 hover:border-moto-accent/50 transition-all duration-500 shadow-sm hover:shadow-xl ${cat.className || 'col-span-1 row-span-1'}`}
                            >
                                ...
                                <div className="absolute inset-0 p-6 flex flex-col justify-between">
                                    <div className="flex justify-between items-start">
                                        <div className="w-10 h-10 rounded-xl bg-white/20 dark:bg-black/30 backdrop-blur-md flex items-center justify-center border border-white/20 group-hover:bg-moto-accent group-hover:text-white transition-colors duration-300">
