                    ...
                    <div className="sticky top-24 space-y-6">
                        {currentUser ? (
                            <div className="bg-[#0A0A0A] border border-white/5 rounded-3xl p-6 relative overflow-hidden group">
                                <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-b from-[#E2FF3B]/10 to-transparent opacity-50" />

                                <div className="relative z-10 flex flex-col items-center text-center">
                                    <div className="w-20 h-20 rounded-full border-2 border-[#E2FF3B] p-1 mb-4 shadow-[0_0_20px_rgba(226,255,59,0.3)]">
                                        <img
                                    ...
                                    <h2 className="text-xl font-display font-bold text-white mb-1">{currentUser.name}</h2>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 w-full mb-6">
                                        <div className="p-3 rounded-2xl bg-white/5 border border-white/5 flex flex-col items-center">
                                            <Gauge className="w-5 h-5 text-[#E2FF3B] mb-2" />
                                            <span className="text-lg font-bold text-white">{currentUser.points || 0}</span>
                                            <span className="text-[10px] text-gray-500 uppercase">SÜRÜŞ PUANI</span>
                                        </div>
                                        <div className="p-3 rounded-2xl bg-white/5 border border-white/5 flex flex-col items-center">
                                            <Navigation className="w-5 h-5 text-blue-400 mb-2" />
                                    ...
                                    <button
                                        ...
                                        className="w-full py-3 rounded-xl bg-white text-black font-bold text-xs hover:bg-[#E2FF3B] transition-colors flex items-center justify-center gap-2"
                                    >
                            ...
                            <div className="bg-[#0A0A0A] border border-white/5 rounded-3xl p-6 text-center">
                                <p className="text-gray-400 mb-4">Giriş yap ve istatistiklerini gör.</p>
                                <button onClick={() => onNavigate && onNavigate('auth')} className="text-[#E2FF3B] font-bold text-sm">Giriş Yap</button>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    >
                        {isLoading ? (
                            Array.from({ length: 3 }).map((_, i) => (
                                <div key={i} className="w-full h-96 bg-[#0A0A0A] rounded-3xl animate-pulse" />
                            ))
                        ) : posts.length > 0 ? (
                            posts.map((post) => (
                                <PostCard
                    ...
                    <div className="sticky top-24 space-y-6">
                        <div className="bg-[#0A0A0A] border border-white/5 rounded-3xl p-6">
                            <h3 className="text-lg font-display font-bold text-white mb-6 flex items-center gap-2">
                                <MapPin className="w-5 h-5 text-[#E2FF3B]" />
                                POPÜLER ROTALAR
                            </h3>
                                ...
                                {[
                                    ...
                                    <div key={i} className="group cursor-pointer">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <h4 className="text-white font-bold text-sm group-hover:text-[#E2FF3B] transition-colors">{route.name}</h4>
                                                <p className="text-xs text-gray-500">{route.dist}</p>
                                            </div>
                                            <button className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-gray-400 group-hover:bg-[#E2FF3B] group-hover:text-black transition-all">
