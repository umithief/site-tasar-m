...
export const PostCard: React.FC<PostCardProps> = ({ post, currentUserId, onNavigate, onCommentClick }) => {
    ...
    return (
        <motion.div
            ...
            className="w-full max-w-[550px] mx-auto bg-[#0D0D0D] rounded-[24px] border border-white/5 shadow-2xl overflow-hidden mb-8 group"
        >
            {/* Header */}
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <div className={`p-2 rounded-full transition-colors ${isLiked ? 'bg-[#FF3E3E]/10' : 'bg-white/5 group-hover/like:bg-white/10'}`}>
                                <Heart
                                    className={`w-5 h-5 transition-all ${isLiked ? 'fill-[#FF3E3E] text-[#FF3E3E]' : 'text-white'}`}
                                />
                            </div>
