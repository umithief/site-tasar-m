...
import { X, Trash2, ShoppingBag, Minus, Plus, Zap, ArrowRight, Truck } from 'lucide-react';
              ...
              <div className="px-6 py-4 bg-white/5 border-b border-white/5">
                  <div className="flex justify-between text-xs mb-2">
                      <span className="text-gray-300 font-bold flex items-center gap-1">
                          {remainingForFreeShipping > 0 ? <><Truck className="w-3 h-3"/> {t('cart.free_shipping')}</> : <><Zap className="w-3 h-3 text-yellow-400"/> {t('cart.free_shipping')}</>}
                      </span>
                      <span className="text-white font-mono">
                          {remainingForFreeShipping > 0 ? t('cart.add_more').replace('{amount}', `₺${remainingForFreeShipping.toLocaleString('tr-TR')}`) : t('cart.reached_limit')}
                      </span>
                  </div>
