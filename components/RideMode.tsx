...
=======
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black text-white flex flex-col font-sans select-none overflow-hidden h-[100dvh]">
            ...
            <div className="absolute inset-0 z-0 overflow-hidden">
                {/* Dark Map Layer */}
                {/* Dark Map Layer with Invert Filter for OSM */}
                <div ref={mapContainerRef} className="w-full h-full grayscale-[0.8] invert contrast-[1.1] brightness-[0.8]" />
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    ...
                    >
                        ...
                        <span className={`text-xs font-mono font-bold hidden md:inline ${isGpsEnabled ? 'text-gray-300' : 'text-red-400'}`}>
                            GPS: {isGpsEnabled ? (gpsStatus === 'active' ? 'ON' : gpsStatus === 'denied' ? 'RED' : gpsStatus === 'unavailable' ? 'YOK' : 'ARA') : 'OFF'}
                        </span>
                    </button>
                </div>
            ...
            {/* Error States */}
            {gpsStatus === 'denied' && (
                <div className="absolute top-24 left-1/2 -translate-x-1/2 z-[70] bg-red-600/90 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-4 max-w-sm backdrop-blur-md border border-red-400/50">
                    <AlertCircle className="w-8 h-8 flex-shrink-0" />
                    <div>
                        <h4 className="font-bold">Konum İzni Reddedildi</h4>
                        <p className="text-xs opacity-90">Lütfen tarayıcı ayarlarından konum erişimine izin verin.</p>
                        <button onClick={() => setGpsStatus('off')} className="mt-2 text-xs underline font-bold">Kapat</button>
                    </div>
                </div>
            )}

            {gpsStatus === 'unavailable' && (
                <div className="absolute top-24 left-1/2 -translate-x-1/2 z-[70] bg-yellow-600/90 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-4 max-w-sm backdrop-blur-md border border-yellow-400/50">
                    <AlertCircle className="w-8 h-8 flex-shrink-0" />
                    <div>
                        <h4 className="font-bold">GPS Sinyali Alınamıyor</h4>
                        <p className="text-xs opacity-90">Cihazınızın GPS'i kapalı veya güvenli bağlantı (HTTPS) yok. Demo Modunu deneyin.</p>
                        <div className="flex gap-3 mt-2">
                            <button onClick={() => setIsDemoMode(true)} className="bg-white/20 px-3 py-1 rounded font-bold text-xs uppercase">Demo Başlat</button>
                            <button onClick={() => setGpsStatus('off')} className="underline font-bold text-xs">İptal</button>
                        </div>
                    </div>
                </div>
            )}

            {!isGpsEnabled && !isDemoMode && gpsStatus !== 'denied' && (
