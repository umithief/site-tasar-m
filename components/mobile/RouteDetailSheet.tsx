    ...
    const MapPreview = () => (
        <div className="w-full h-48 bg-zinc-900 rounded-2xl overflow-hidden relative mb-6 border border-white/10 group">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/40">
                <span className="bg-black/80 backdrop-blur text-white text-xs px-3 py-1 rounded-full border border-white/10">Preview Map</span>
            </div>
        </div>
    );

    const StatGauge = ({ label, value, icon: Icon, color = 'text-orange-500' }: any) => (
        <div className="flex flex-col items-center flex-1 bg-zinc-900/50 p-3 rounded-2xl border border-white/5">
            ...
            {route && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed bottom-0 left-0 right-0 bg-[#0F0F11] rounded-t-[32px] overflow-hidden z-[51] h-[85vh] border-t border-white/10 flex flex-col"
                    >
                        {/* Drag Handle */}
                        ...
                        <div className="flex-1 overflow-y-auto p-6 pt-2 pb-32 no-scrollbar">
                            ...
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${route.difficulty === 'Extreme' ? 'bg-red-500/20 text-red-500' : 'bg-green-500/20 text-green-500'}`}>
                                            {route.difficulty}
                                        </span>
                                    ...
                                    <h2 className="text-3xl font-bold text-white leading-tight">{route.title}</h2>
                                </div>
                            ...
                            {/* Key Stats Row */}
                            <div className="flex justify-between items-center mb-8 px-4 py-4 bg-zinc-900 rounded-2xl border border-white/5">
                                <div className="text-center">
                                    <div className="text-2xl font-mono font-bold text-white">{route.distance}</div>
                                    <div className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider">Distance</div>
                                </div>
                                <div className="w-px h-8 bg-zinc-800" />
                                <div className="text-center">
                                    <div className="text-2xl font-mono font-bold text-white">{route.estimatedTime}</div>
                                    <div className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider">Duration</div>
                                </div>
                                <div className="w-px h-8 bg-zinc-800" />
                                <div className="text-center">
                                    <div className="text-2xl font-mono font-bold text-orange-500">{route.riderCount}</div>
                                    <div className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider">Riders</div>
                                </div>
                            </div>
