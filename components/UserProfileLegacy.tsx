...
import { User as UserType, Order, ViewState, UserBike as UserBikeType, SocialPost } from '../types';
...
export const UserProfile: React.FC<UserProfileProps> = ({ user, onLogout, onUpdateUser, onNavigate }) => {
    const { t } = useLanguage();
                ...
                <div className="flex p-1 bg-white border border-gray-200 rounded-2xl mb-8 overflow-x-auto no-scrollbar shadow-sm">
                    {[
                        ...
                        <button
                            ...
                            className={`flex-1 py-3 px-4 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all relative ${activeTab === tab.id ? 'text-black' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'
                                }`}
                        >
                            {activeTab === tab.id && (
                                <motion.div
